...
const parseAnsi = (input: string, cols: number, rows: number): ParsedAnsi => {
  const grid: Array<AnsiCell[] | null> = Array.from({ length: rows }, () => null);
  let row = 0;
  let col = 0;
    ...
    if (match[1] !== undefined && match[2]) {
      ...
      if (command === "m") {
        ...
        params.forEach((code) => {
          ...
          if (ANSI_BG_MAP[code]) currentStyle.backgroundColor = ANSI_BG_MAP[code];
...
const condenseRow = (row: AnsiCell[] | null) => {
  ...
  const trimmedRow = lastNonSpaceIndex >= 0 ? row.slice(0, lastNonSpaceIndex + 1) : [];
...
export const AnsiMessageRenderer = ({
  text,
  ...
  const startRow = Math.max(0, Math.floor(scrollTop / lineHeight) - 10);
  const endRow = Math.min(
    visibleRows.length,
    Math.ceil((scrollTop + viewportHeight) / lineHeight) + 10,
  );
  const slice = visibleRows.slice(startRow, endRow);

  return (
    <div className={className} ref={containerRef} style={{ overflow: "auto" }}>
      <div style={{ height: totalHeight, position: "relative" }}>
        <div style={{ transform: `translateY(${startRow * lineHeight}px)` }}>
          {slice.map((row, index) => {
            const rowIndex = startRow + index;
            const segments = condenseRow(row);
            return (
              <div key={rowIndex} style={{ height: lineHeight }}>
